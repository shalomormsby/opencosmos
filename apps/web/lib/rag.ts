...
Each quote carries its own "Cite as" line too. When you draw on a quote, append that token — exactly as given — immediately after the attribution. Never assemble a quote token yourself, and never cite a quote that wasn't retrieved above: a token you construct points at a record that does not exist, and the reader gets a broken link. If you want to mention a quote that isn't in the retrieved set, simply attribute it in prose with no token. If a quote's provenance status is anything other than \`verified\`, soften your attribution language ("attributed to X", "popularly attributed to X") rather than asserting "X said". Never present a quote whose status is \`likely_misattributed\` or \`apocryphal\` without flagging the doubt.`
