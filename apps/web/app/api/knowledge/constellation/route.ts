...
export async function GET() {
  const compressed = await redis.get<string>('knowledge:constellation')

  if (!compressed) {
    return new Response('Constellation graph not yet generated. Run pnpm graph:constellation to populate.', {
      status: 404,
    })
  }
