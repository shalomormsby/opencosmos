...
## 2026-08-21 — Feature: the quote substrate comes alive (provenance for all 1,509 · quotes get pages · citations that work)
...
- **Verified** end to end against the running app: all 133 citable quotes at the time of the check resolved to a page carrying their anchor id (0 unresolvable, 0 non-200, 0 missing anchors); path traversal and malformed buckets produce no link; an unknown bucket returns 404 with a helpful page. `pnpm quotes:lint` green after every mutation, pools reconciling to 1,509 throughout. `tsc --noEmit` clean. **Not visually verified in a browser** — HTTP-level assertions only — and **not yet re-embedded**: Upstash's 10k daily write cap is exhausted, so the live index still holds a 133-quote state. Run `pnpm embed` when it resets to make all 349 citable.
